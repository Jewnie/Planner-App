...
import {
  listEventsByAccountId,
  getCalendarProvidersForAccount,
  getCalendarsForProviders,
  checkIfSyncIsRunning,
  createAllDayEvent,
  createTimedEvent,
  assertUserHasWritePermissionToCalendar,
  deleteEvent,
...
export const calendarRouter = router({
  listCalendars: protectedProcedure.query(async ({ ctx }) => {
    ...
    const userCalendars = await getCalendarsForProviders({
  ...
  createAllDayEvent: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      await assertUserHasWritePermissionToCalendar({
        accountId: ctx.accountId,
        calendarId: input.calendarId,
      });

      console.log('Creating all day event', input);
      return await createAllDayEvent({
        accountId: ctx.accountId,
        calendarId: input.calendarId,
        title: input.title,
