...
export const assertCalenderHasProvider = async (params: {
  accountId: string;
  calendarId: string;
}) => {
  const provider = await db
    .select({ id: calendarProviders.id, providerType: calendarProviders.name })
    .from(calendarProviders)
    .where(and(eq(calendarProviders.accountId, params.accountId)));
  ...
  const calendar = await db
    .select({ id: calendars.id, calendarProviderCalendarId: calendars.providerCalendarId })
    .from(calendars)
    .where(and(eq(calendars.providerId, provider[0].id), eq(calendars.id, params.calendarId)));
  ...
  return {
    ...
    providerType: provider[0].providerType,
    calendarProviderCalendarId: calendar[0].calendarProviderCalendarId,
  };
};
...
export async function getCalendarsForProviders(params: { providerIds: string[] }) {
  return db.select().from(calendars).where(inArray(calendars.providerId, params.providerIds));
}
...
export const listEventsByAccountId = async (
  ...
  const rangeConditions = dateRanges.map(({ startDate, endDate }) =>
    and(lt(events.startTime, endDate), gt(events.endTime, startDate)),
  );
  ...
  for (const event of recurringEvents) {
    ...
    try {
      ...
      const occurrenceDates = rule.between(overallStart, overallEnd);
...
export const getExistingCalendarWatchDataForCalendar = async (params: {
  calendarId: string;
  providerId: string;
}) => {
  return await db
    .select({
      id: calendarWatches.id,
      channelId: calendarWatches.channelId,
      resourceId: calendarWatches.resourceId,
    })
...
export const createAllDayEvent = async (params: {
  ...
  if (params.calendarId) {
    const calendarProvider = await assertCalenderHasProvider({
  ...
  if (params.calendarId && calendarProviderCalendarId) {
    // TODO: update this when users can create internal calendars
    console.log('notifying provider of created event');
    await notifyProviderOfCreatedEvent({
      dbEvent: dbEvent[0],
      accountId: params.accountId,
      calendarProviderCalendarId: calendarProviderCalendarId,
    });
  }
...
export const notifyProviderOfCreatedEvent = async (params: {
  dbEvent: InferSelectModel<typeof events>;
  accountId: string;
  calendarProviderCalendarId: string;
}) => {
  ...
  const startTime = params.dbEvent.allDay
    ? params.dbEvent.startTime.toISOString().slice(0, 10)
    : params.dbEvent.startTime.toISOString();
  const endTime = params.dbEvent.allDay
    ? params.dbEvent.endTime.toISOString().slice(0, 10)
    : params.dbEvent.endTime.toISOString();
  const providerResponse = await calendarClient.events.insert({
    ...
    requestBody: {
      ...
      start: params.dbEvent.allDay
        ? { date: startTime }
        : { dateTime: startTime, timeZone: params.dbEvent.timeZone },
      end: params.dbEvent.allDay
        ? { date: endTime }
        : { dateTime: endTime, timeZone: params.dbEvent.timeZone },
    },
  });
  if (providerResponse.status === 200) {
    console.log('updating provider event id');
    await updateEventProviderEventId({
      providerEventId: providerResponse.data.id as string,
      eventId: params.dbEvent.id,
    });
...
export const updateEventProviderEventId = async (params: {
  providerEventId: string;
  eventId: string;
}) => {
  await db
    .update(events)
    .set({ providerEventId: params.providerEventId })
    .where(eq(events.id, params.eventId));
};

export const assertUserHasWritePermissionToCalendar = async (params: {
  accountId: string;
  calendarId: string;
}) => {
  const provider = await assertCalenderHasProvider({
    accountId: params.accountId,
    calendarId: params.calendarId,
  });
...
export const assertEventExists = async (params: {
  accountId: string;
  eventId: string;
  calendarId: string;
}) => {
...
export const deleteEvent = async (params: {
  ...
  const { provider, calendar } = await assertUserHasWritePermissionToCalendar({
    accountId: params.accountId,
    calendarId: params.calendarId,
  });
  const event = await assertEventExists({
    accountId: params.accountId,
  ...
  await db
    .delete(events)
    .where(and(eq(events.id, params.eventId), eq(events.calendarId, params.calendarId))); // TODO: SOFTDELETE VS HARDDELETE?
};
